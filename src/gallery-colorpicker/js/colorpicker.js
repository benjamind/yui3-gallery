	...
	*/ 
	...
	ColorPicker.CLASSNAME_SQUARE = 'hslpColrsqr';
	ColorPicker.CLASSNAME_HBAR = 'hslpHbar';
	ColorPicker.CLASSNAME_SBAR = 'hslpSbar';
	ColorPicker.CLASSNAME_LBAR = 'hslpLbar';
	ColorPicker.CLASSNAME_SWATCH = 'hslpSwatch';
	ColorPicker.CLASSNAME_VALUE = 'hslpVal';
	ColorPicker.CLASSNAME_HVALUE = 'hslpHval';
	ColorPicker.CLASSNAME_SVALUE = 'hslpSval';
	ColorPicker.CLASSNAME_LVALUE = 'hslpLval';
	ColorPicker.CLASSNAME_RVALUE = 'hslpRval';
	ColorPicker.CLASSNAME_GVALUE = 'hslpGval';
	ColorPicker.CLASSNAME_BVALUE = 'hslpBval';
	ColorPicker.CLASSNAME_HEXVALUE = 'hslpHexval';
	...
	Y.extend(ColorPicker, Y.Widget, {
		...
		onMouseMove: function (ev) {
			...
			if (this.trackSquare) {
				offXY = Y.Node.get(this.squareCanvas).getXY();
				// get position within the square
				x = ev.clientX - offXY[0];
				y = ev.clientY - offXY[1];
		...
		renderSquare: function () {
			...
			ctx.strokeStyle = 'rgb(255, 255, 255)';
			radius = 5;
			x = (this.color.h * width);
			y = ((1.0 - this.color.l) * height);
			ctx.beginPath();
		...
		bindUI: function () {
			...
			Y.delegate('keyup', this.onInputKeyUp, this.contentBox, 'div.hslpVal', this);
			Y.delegate('blur', this.onInputBlur, this.contentBox, 'div.hslpVal', this);
			Y.delegate('keydown', this.onInputKeyDown, this.contentBox, 'div.hslpVal', this);
		},
		syncUI: function () {
			var width = this.get('squareWidth'), height = this.get('squareHeight');
			this.squareX = this.color.h + width;
			this.squareY = this.color.l + height;
			this.update();
		}
	});
	...
	ColorPalette.CLASSNAME_SWATCH = 'paletteSwatch';
